...
class Parser:
    ...
    def predict(self,Sentence):
        ...
        Sentence = ' '.join(key for key in Sentence)
        ...
        finalPred = list(zip(words,pred))
        
        engKey = ' '.join(key for key,val in finalPred)
        engKey = re.sub("\u200e|\u200f","",engKey)
        engKey = engKey.split(" ")
        
        
        engVal = ' '.join(val for key,val in finalPred)
        engVal = engVal.split(" ")
        
        
        finalPred = [list(i) for i in zip(engKey,engVal)]
        return finalPred
